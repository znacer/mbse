<script lang="ts">
	...
	let { transform, width: canvasWidth, height: canvasHeight, onNavigate, nodes: simNodes = [] }: Props = $props();
	...
	function getBounds() {
		if (simNodes.length === 0) return { minX: -200, maxX: 200, minY: -150, maxY: 150 };
	...
	function getLinkPositions() {
		const bounds = getBounds();
		const scale = getScale();
		return appState.graphLinks.map(l => {
...
<div class="absolute bottom-4 right-4 bg-[#0a0f14]/95 border border-[#1a2530] rounded shadow-lg overflow-hidden">
	<div class="px-2 py-1 text-xs font-medium text-[#00ff88]/70 border-b border-[#1a2530] bg-[#0d1318]">
		TACTICAL MAP
	</div>
	<svg
		...
		class="cursor-pointer bg-[#0a0f14]"
		role="button"
		...
		onkeydown={(e) => {
			if (e.key === 'Enter' || e.key === ' ') {
				const bounds = getBounds();
				const scale = getScale();
				onNavigate((minimapWidth / 2) / scale + bounds.minX, (minimapHeight / 2) / scale + bounds.minY);
			}
		}}
	>
		<defs>
		...
		<rect width={minimapWidth} height={minimapHeight} fill="url(#minimap-grid)" />
